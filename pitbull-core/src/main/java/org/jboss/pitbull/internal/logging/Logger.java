package org.jboss.pitbull.internal.logging;
...
public abstract class Logger
{
   ...
   public static void setLoggerType(LoggerType loggerType)
   {
      try
      {
         ...
         {
            loggerClass = Thread.currentThread().getContextClassLoader().loadClass("org.jboss.pitbull.internal.logging.impl.JULLogger");
         }
         else if (loggerType == LoggerType.LOG4J)
         {
            loggerClass = Thread.currentThread().getContextClassLoader().loadClass("org.jboss.pitbull.internal.logging.impl.Log4jLogger");
         }
         else if (loggerType == LoggerType.SLF4J)
         {
            loggerClass = Thread.currentThread().getContextClassLoader().loadClass("org.jboss.pitbull.internal.logging.impl.Slf4jLogger");
         }
         if (loggerClass == null)
